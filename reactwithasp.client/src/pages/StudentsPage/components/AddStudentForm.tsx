...
type AddStudentFormProps = { student: IStudent | undefined; storeStudent: (data: IStudent) => void }
    ...
    return (
        <form onSubmit={handleSubmit(storeStudent)} className='flex flex-col gap-3'>
            ...
            <div>
                ...
                <input id="email"
                    className={formStyle.input} type="email"
                    {...register("email", {required: true})}
                    defaultValue={student?.email || ''} />
            </div>
            <button className={formStyle.button} type="submit">Pridėti studentą</button>
        </form>
    )
}